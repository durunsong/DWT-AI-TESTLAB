...
export function buildCaseGenerationPrompt(requirement: string): string {
  return [
    ...
    "生成内容必须包含 case_id、case_name、mode、defaults、sessions、locations.file 和 steps。",
    "defaults 必须包含 step_timeout_ms: 20000 和 wait_for_network: true，除非用户明确要求调整。",
...
export function buildMaterialCaseGenerationPrompt(input: MaterialCaseGenerationPromptInput): string {
  ...
  return [
    ...
    "YAML 必须能被平台 DSL 校验：包含 case_id、case_name、mode、defaults、sessions、locations.file、steps。",
    "必须在顶层输出 defaults: { step_timeout_ms: 20000, wait_for_network: true }，除非用户明确要求调整。",
    ...
    "只有用户明确要求数据库核验，且资料给出了表名、字段、查询条件和可用参数时，才在 UI 操作后追加 db_assert；信息不完整时使用成功提示或 wait_for_api，不要臆造数据库校验。",
    "db_query/db_assert 字段规范：sql 写参数化只读 SQL，params 写数组参数，expected 可写字符串或字段对象，save_as 可把 db_query 的单列值或整行 JSON 写入 ${var.xxx}，row_index 从 0 开始；DB 步骤不要写 session。",
    "db_query/db_assert 的 params 只能引用已在 sessions 中出现过的 env，或 YAML 顶层 variables 中定义过的 var；不要臆造 ${env.ADMIN_USER_ID}、${env.USER_ID} 等未配置环境变量。",
    "定位字段必须使用 target；输入字段必须使用 value；上传文件字段必须使用 file；文本断言字段必须使用 expected。不要输出 locator、selector、file_path 或 text 字段。",
    "target 优先使用 locations.file 中已有定位 key；直接写 key 名（如 admin_profile_username），不要写 ${loc.admin_profile_username}；没有 key 时可使用页面可见文案；确需选择器时可直接写 XPath（如 //span[text()='系统']）或 CSS（如 .el-menu-item），不要把选择器写到 selector/locator 字段。",
    "step.type 只能使用：web_open、web_reload、web_input、web_click、web_upload、web_wait_text、web_wait_element、web_assert_text、web_assert_visible、web_assert_url、web_extract、web_screenshot、flow_login、flow_submit_kyc、flow_admin_approve_kyc、api_request、api_assert、db_query、db_assert。不要生成 db_clean。",
...
export function buildCaseYamlAssistPrompt(input: CaseYamlAssistPromptInput): string {
  ...
  return [
    ...
    "target 应优先复用 locations.file 中已有定位 key，直接写 key 名（如 admin_profile_username），不要写 ${loc.admin_profile_username}；没有 key 时可用页面文案，必要时可直接使用 XPath/CSS；输入字段必须使用 value，不要使用 text；不要臆造生产密钥或真实账号。",
    "admin 个人信息/修改资料场景登录后直接打开 ${session.login_url}#/admin/sys/perinfo，并复用 admin_profile_username、admin_profile_password、admin_profile_save、admin_profile_success_message；若有头像附件，保存前新增 web_upload，target 使用 admin_avatar_upload；不要生成点击“系统”“个人信息”菜单或等待“编辑个人信息”的脆弱 XPath。",
    "保存、提交、审核等动作如知道后端接口，优先在触发步骤上补 wait_for_api，例如 wait_for_api: { method: POST, url: /xxx/edit, expected_status: 200, business_code_path: code, success_codes: [\"0000\"] }；业务成功码可由 API_BUSINESS_SUCCESS_CODES 配置。",
    "不要把“当前页面仍可见”“菜单仍可见”当作保存/提交成功断言；应改为成功提示、接口业务成功、列表状态变化或 db_assert。",
    "只有用户明确要求数据库核验，且当前 YAML 或用户说明给出了表名、字段、查询条件和可用参数时，才使用 db_assert；DB 步骤不要写 session，不要臆造 ${env.ADMIN_USER_ID}、${env.USER_ID} 等未配置环境变量。",
